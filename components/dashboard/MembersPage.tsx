...
import { GroupInfo, GroupInfoPerson } from '@/types';
  ...
  return (
    <div className={styles.wrap}>
      ...
      <div className={styles.actions}>
        <button className={styles.btnPrimary}>Create Family</button>
        <button className={styles.btnGhost}>Edit Family</button>
      </div>
      ...
      <div className={styles.content}>
        {familyGroups.map(group => (
          <div className={styles.familySection} key={group.familyId}>
            <div className={styles.familyHeader}>
              {group.familyName}
              <span className={styles.familyCount}>{group.members.length} member{group.members.length !== 1 ? 's' : ''}</span>
            </div>
            {group.members.map(member => (
              <div className={styles.memberRow} key={member.userId}>
                ...
                <span className={styles.familyTag}>{group.familyName}</span>
        ...
        {noFamily.length > 0 && (
          <div className={styles.familySection}>
            <div className={styles.familyHeader}>
              No family
              <span className={styles.familyCount}>{noFamily.length} member{noFamily.length !== 1 ? 's' : ''}</span>
            </div>
            {noFamily.map(member => (
              <div className={styles.memberRow} key={member.userId}>
                <div className={styles.avatar} aria-hidden="true">{(member.displayName[0] ?? '?').toUpperCase()}</div>
